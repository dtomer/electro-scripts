...
foreach $i (@_) {
  print "Atom $i is not defined or calculated: check the input or the loc file" if (not defined $ind[$i]);
  $i=$ind[$i]; 
...
printf "FLU = %10.6f\n",  $FLU/$NRing;
 ...
 open RESULTS, ">>aromaticity";
 printf RESULTS "%-9.4f %-9.4f %-9.4f   ", $FLU/$NRing, $average, $std;
 for $i (0..$NRing-1) {
    print RESULTS "$AtName[$_[$i]]$NumAt[$_[$i]] ";
}
# print RESULTS "\n";
